...
实测倍数：单请求口径（API 上限内）**2.57×**（25.80 s → 10.06 s），整 dev split **2.27×**
（84.2 s → 37.0 s），argmax 一致率 100%。前向怎么排、这些数字背后的形态依赖，都在
...
| 架构 | 骨干 | 状态 |
|---|---|---|
| `nanojev` | Qwen3-0.6B 因果解码器 + 训练好的决策头 | 默认 |
| `laya` | ModernBERT 编码器，state 与选项同在一段序列里 | 已声明 |
| `decider-2b` | Qwen3.5-2B 因果解码器，每 4 层有 3 层线性注意力 | 已声明 |

目前只有 `nanojev` 接通；其余两个会拒绝运行。各架构的**结构性事实**（注意力怎么走、答案从哪里
读出、段与段之间要驻留什么）记在 `src/jevinf/arch.py` —— 三阶段编排是否适用，就是由这些事实决定的。
